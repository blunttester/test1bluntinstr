...
function themify_hide_shop_title() {
	if('' != themify_get('setting-hide_shop_title') && is_shop()){
		add_filter('woocommerce_page_title', create_function('', 'return "";'));
	}
};
...
function themify_theme_add_to_cart_fragments( $fragments ) {
	global $woocommerce;
	// cart list
	ob_start(); get_template_part( 'includes/shopdock' );
	$fragments['#shopdock'] = ob_get_clean();
	$fragments['#cart-icon .amount'] = $woocommerce->cart->get_cart_total();
