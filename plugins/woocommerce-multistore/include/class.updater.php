...
class WOO_MSTORE_CodeAutoUpdate {
	...
	public function prepare_request( $action, $args = array() ) {
        if ( function_exists('is_multisite') && is_multisite() === false && get_option('woonet_network_type') == 'child' ) {
    ...
    public function prepare_request_for_single_site( $action, $args = array() ) {
        ...
        $data = $_engine->get_data_for_update();
        $json = woomulti_json_decode($data);
        
        if ( !empty($json->key) && !empty($json->domain) ) {
            return array(
                'woo_sl_action'     => $action,
                'version'           => WOO_MSTORE_VERSION,
                'product_unique_id' => WOO_MSTORE_PRODUCT_ID,
                'licence_key'       => $json->key,
                'domain'            => $json->domain,
                'wp-version'        => $wp_version,
            );
        }
