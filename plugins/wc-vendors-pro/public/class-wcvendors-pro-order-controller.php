...
class WCVendors_Pro_Order_Controller {
	...
	public function table_rows() {
		 ...
		 * @since 1.7.10
		 */
		$hide_reversed_orders = apply_filters( 'wcvendors_hide_reversed_orders', true );

		$all_orders = WCVendors_Pro_Vendor_Controller::get_orders2( get_current_user_id(), $date_range, $hide_reversed_orders );
		...
		if ( ! empty( $all_orders ) ) {
			...
			foreach ( $all_orders as $_order ) {
				...
				do_action_deprecated( 'wcv_orders_add_new_row', array( $new_row, $_order, $order, $_order->order_items ), '1.7.10', 'wcvendors_orders_add_new_row' );
				do_action( 'wcvendors_orders_add_new_row', $new_row, $_order, $order, $_order->order_items );
