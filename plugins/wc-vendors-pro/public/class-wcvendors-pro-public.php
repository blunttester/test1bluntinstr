	...
	public function enqueue_styles() {
		...
		if ( wcv_is_dashboard_page( $current_page_id ) || ( $view_feedback ) || ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'wcv_pro_dashboard_nav' ) ) || ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'wcv_pro_dashboard' ) ) ) {
			...
			wp_enqueue_style(
				...
				$this->base_dir . 'assets/lib/flatpickr/flatpickr.min.css',
				array(),
				WCV_PRO_VERSION
			);
		}
	...
	public function enqueue_scripts() {
		...
		if ( wcv_is_dashboard_page( $current_page_id ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				wp_register_script(
					'wcv-flatpickr',
					$this->base_dir . 'assets/lib/flatpickr/flatpickr.js',
					array(),
					WCV_PRO_VERSION,
					true
				);
				...
				wp_register_script(
					'wcv-datetime-picker',
					$this->base_dir . 'assets/js/datetime-picker.js',
					array( 'wcv-flatpickr' ),
					WCV_PRO_VERSION,
					true
				);
				...
				wp_enqueue_script( 'wcv-flatpickr' );
