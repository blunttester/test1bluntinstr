...
class WCVendors_Pro_Admin {
	...
	public function enqueue_scripts() {
		...
		$screen_id = $screen->id;
		$product   = 0;
		$_current_page = isset( $_GET['page'] ) ? sanitize_text_field( $_GET['page'] ) : '';
		...
		if ( 'wc-vendors_page_wcv-settings' == $screen->id || 'product' == $screen->id || 'user-edit' == $screen->id || 'wcv-settings' == $_current_page ) {
	...
	public function admin_lockout() {
		...
		if ( 'yes' == get_option( 'wcvendors_disable_wp_admin_vendors', 'no' ) ) {
			...
			$roles = array( 'vendor', 'pending_vendor' );

			foreach ( $roles as $role ) {

				if ( in_array( $role, $user_roles, true ) && ! defined( 'DOING_AJAX' ) ) {
