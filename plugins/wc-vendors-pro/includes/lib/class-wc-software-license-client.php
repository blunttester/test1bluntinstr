 ...
 * @author         Jamie Madden <github.com/digitalchild>
 */
...
if ( ! class_exists( 'WC_Software_License_Client' ) ) :
	...
	class WC_Software_License_Client {
		...
		/**
		 * Renewal URL - the url to renew the product
		 *
		 * @since 1.0.3
		 */
		public $renewal_url;
		 ...
		 */
		public static function get_instance( $license_server_url, $renewal_url, $version, $text_domain, $plugin_file, $plugin_nice_name, $slug = '', $update_interval = 12, $debug = false ) {

			if ( ! array_key_exists( $text_domain, self::$instances ) ) {
				self::$instances[ $text_domain ] = new self( $license_server_url, $renewal_url, $version, $text_domain, $plugin_file, $plugin_nice_name, $slug, $update_interval, $debug );
			}
		 ...
		 * @param integer $update_interval    - time in hours between update checks
		 */
		private function __construct( $license_server_url, $renewal_url, $version, $text_domain, $plugin_file, $plugin_nice_name, $slug, $update_interval, $debug ) {
		...
		public function check_install() {
			...
			} elseif ( $this->license_details['license_status'] == 'expired' ) {
		...
		public function license_inactive() {
			...
			if ( ! in_array( $screen->id, wcv_get_screen_ids() ) && 'plugins' !== $screen->id ) {
				return;
			}
			...
			echo '<div class="error notice is-dismissible"><p>' .
				 sprintf( __( 'The %1$s license key has not been activated, so you will be unable to get automatic updates or support! %2$sClick here%3$s to activate your support and updates license key.', 'wcvendors-pro' ), $this->plugin_nice_name, '<a href="' . $this->license_manager_url . '">', '</a>' ) .
				 '</p></div>';

		} // license_inactive()

		/**
		 *    Display the license has expired code
		 */
		public function license_expired() {
			...
			echo '<div class="error notice is-dismissible"><p>' .
				 sprintf( __( '<strong>The %1$s license key has expired, you will be unable to get automatic updates or support! </strong> %2$sClick here%3$s to purchase your new support and updates license key.', 'wcvendors-pro' ), $this->plugin_nice_name, '<a href="' . $this->renewal_url . '">', '</a>' ) .
				 '</p></div>';
		}
		...
		public function validate_license( $input ) {
			...
			foreach ( $options as $key => $value ) {
				...
				if ( 'license_key' === $key ) {
					...
					// Only run this code if the license hasn't expired
					if ( $this->license_details['license_status'] !== 'expired' ) {
