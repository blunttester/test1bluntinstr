...
namespace WpPhpMyAdminExtension
{
  ...
  class PluginClass
  {
	...
	public function setup_files()
	{
		...
		$this->pma_delete_dirs		= array('/vendor/tecnickcom/tcpdf', '/locale', '/themes/original', '/doc', '/setup', '/examples', '/install', '/js/vendor/openlayers' );	  //vendor\phpmyadmin\sql-parser\locale
			...
			{ 
				if( is_dir($this->pma_abspath_real) && !is_dir($this->pma_abspath) ) //check again after LOCK 
				{
					...
					if ( stripos($cont,$flag) === false )
					{
						...
						$common_inc_content_new  = $this->str_replace_first( '<?php', '<?php '.$addition, $cont );
					...
					// rename conflicting function named __(    //old method: pastebin(dot)com/raw/v652Ef1A
			...
			{ 
				if(isset($_GET['hosting_pma'])){
					...
					if(empty($_COOKIE[$sess_vars["name"]]) || $_COOKIE[$sess_vars["name"]] != $sess_vars["value"] ){
						$hours = 3*60*60;
						$this->set_cookie( $sess_vars["name"], $sess_vars["value"], $hours);
						$this->set_cookie( "pma_DB_NAME", DB_NAME, $hours);
						$this->php_redirect();
					}
