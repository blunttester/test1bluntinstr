...
class standard_php_library__PuvoxSoftware 
{
	...
	public function __construct($args=['FILE'=>__FILE__, 'NAMESPACE'=>"EXAMPLE"])
	{
		...
		$isClass=(array_key_exists('class',$args));
		$reflection				= $isClass ? (new \ReflectionClass( $args['class'] )) : null;
		$this->module_NAMESPACE	= $isClass ? $reflection->getNamespaceName() : $args['NAMESPACE']; 				// get parent's namespace name
		$this->moduleFILE		= $isClass ? $reflection->getFileName() : $args['FILE']; 		// set plugin's main file path
		...
		$this->changeable_JS_CSS_version =  (file_exists($file = $this->moduleDIR.'/style-public.css') ? 'date_'.filemtime($file) : $this->sanitize_key($this->domainReal).date('m') );
	...
	public function sanitize_nonalpha($input){  return strtr($input, [
		" "=>"-",	"."=>"--",	":"=>"--",	","=>"-",	"/"=>"-",	";"=>"--",	"—"=>"",	"–"=>"-"
		]);
	}
	...
	// because of WP rquest , removed original function [ https://github.com/ttodua/useful-php-scripts/blob/master/get-remote-url-content-data.php ]
	public function get_remote_data($url, $post_paramtrs=false,             $request_options=[])	
	{
		...
		{
			if($func=="wp_remote_get") return call_user_func_array($func, [$url] )['body'];
			if($func=="wp_remote_post") return call_user_func_array($func, [$url, $post_array] )['body'];
		}
		return "empty_data. Create your own remote function";
	}
	
	// removed: https://pastebin_com/jFpk0Cb2
	// 		set_cookies_from_url
	// 		multi_curl
	//		curlFast
	
	// removed: https://pastebin_com/Kva9Ez8c
	// 		send_data_to_pastebin
	//		create_pastebin
	// 		create_gist
	...
	// ==================== text to image==============
	// at first, download unicode TruType font like: https://github.com/edx/edx-certificates/raw/master/template_data/fonts/Arial%20Unicode.ttf
	// # Usage #
	//TextToImage_my( 
	//	$text='Helloooo World!' , 
	...
	public function TextToImage($text, $separate_line_after_chars=40,  $font='./Arial%20Unicode.ttf.ttf', 
	...
	public function js_library($url_or_Tag=true, $defaultPath=""){
		if( empty($defaultPath) && function_exists('home_url') )
			$defaultPath = home_url() . '/wp-content/plugins/';
		$url = $defaultPath . basename(__FILE__).'?JS';
	...
	public function get_youtube_data($viid_id, $part='')	{ 
		...
		$data = file_get_contents($ResponserUrl.$viid_id);
	...
	public function yout_DownUrls($viid_id, $titlee='')	{ 
		...
		if (!empty($full_info['player_response'])) 	{
			$array = json_decode($full_info['player_response'], true);		
			$VideoAudioStreams = $array['streamingData']['formats'];	 	// $array['adaptiveFormats'] -->has muted videos
			$targetKey = array_search(1280, array_column($VideoAudioStreams, 'width')); 
			$targetVideo = $VideoAudioStreams[$targetKey]['url'];
			$vidLink= $targetVideo.'&title='.$array['videoDetails']['title'].'_'; 
			
			$AudioStreams = $array['streamingData']['adaptiveFormats'];
			$url = array_search('44100', array_column($AudioStreams, 'audioSampleRate', 'url'));
			$targetAudio = $url .'&title='.$array['videoDetails']['title'].'_';
			$audLink = $targetAudio;
		} 
	...
	public function Download_Filee( $allowed_directory="/wp-content/uploads/myFiles", $filename  )
	{
		...
		if (!file_exists($path)){ echo '<b style="background:red;">error_750 [contact administrator !!]</b><pre>'; var_dump($FullFile);echo '</pre>';exit;}
	...
	public function error_notify_admin__MYDDD($error_msg=false,$postidd=false){ 	if (test_environment) return;
		...
		//write into file
			//$file=$this->moduleDIR.'/zzz___ajax_error_notifications_'.$this->my_site_variables__secret('rand_name', RandomString(11)).'.txt';
	...
	//sample: add_action('wp_enqueue_scripts', function(){wp_deregister_script('jquery'); wp_enqueue_script('jquery', 'https://[code.jquery.com]/jquery-'.'3.3.1'.'.min.js', [], null, true); }  );
	...
	public function init_load_scripts()
	{
	...
	{
		$array=[];
		if (!empty($argv_[1])) parse_str($argv_[1], $array);	
		return $array;
	}
...
//==========================================================================================================
//==========================================================================================================
//======================================== 2) Library of JS functions  =====================================
//==========================================================================================================
//==========================================================================================================

#region 2

function PuvoxSoftware_JS()
{
	session_cache_limiter('none');
	...
	header("Content-type: application/javascript;  charset=utf-8");
	?>
	//  this "<script>" tag just starts JAVSCRIPT highlighting in text-editor, so don't remove this line.
		
	// ************ useful scripts got from: https://github.com/ttodua/useful-javascript/   **********

var PuvoxSoftware_JS = 
{
	this_ : this,
	...
	Append_To_Head(elemntType, content){
		...
		if(Is_Link){
			//assign temporary id
			var id= encodeURI( content.split('/').reverse()[0] );
	...
	//equivalent to wp sanitize_key
	sanitize_key (str)
	{
		return str.toLowerCase().replace(/[\W_]+/g,"-");  //replace("(","-").replace(")","-").replace(/\s/g,"-").replace(/--/g,"-");
	},
	...
	animatecss_click(element, animation){
	  element = $(element);
	  element.click(
		function() {
	...
	animationClick(element, animation){animatecss_click(element, animation);},
	...
	jquery_popup(element, isModal, params){
		...
		var width = params.hasOwnProperty('width') ? params.width : 300;
		...
		return $(element).clone().dialog(
		{
			...
			open: function() {
				...
				$('body').append('<div id="custom_overlay"><style>.ui-widget-overlay {opacity:0.8;}</style></div>');
	...
	SmoothScrollTo(id_or_Name, timelength, setHashInAddress){
		var timelength = timelength || 1000;
		var setHashInAddress = setHashInAddress || false;
		$('html, body').animate({
	...
	CreateFrameIn(targetEl, frameContent, MakeItfullWH){	
		...
		iframe.className="innerFrame innerFrame"+RandomNum(999999);
	...
	//=====   http://spin.js.org/   // https:// cdnjs cloudflare com /ajax/libs/spin.js/2.3.2/spin.js
	InitializeSpinner(){
		var opts = {
	...
	spin(action){
		...
		if(action=="show"){
			$("body").append('<div id="blackground_spinner" style="width:100%;height:100%;position:fixed;background:white; z-index:3333; opacity:0.8; top:0px;left:0px;"></div>');
	...
	SHOW_waiting(style, content_To_show){	
		var b=document.body;	
		content_To_show= content_To_show || "LOADING...";
		if(content_To_show.substring(0,4)=='http' || content_To_show.substring(0,1)=='/'){ 
			content_To_show = '<img class="innerLoad" src="'+content_To_show+'" alt="loading"  style="width:auto; max-width:70%; height:auto; max-height:80%; " />'; 
		}
		var x = document.createElement('div'); x.id = 'my_waiting_box';    x.setAttribute("style","height:100%; width:100%; position:fixed; z-index:9900;");

		x.innerHTML = 
		'<div id="my_waiting_background" style="background-color: black; height: 100%; position: absolute; opacity: 0.9; width: 100%;' + (style || '') + '"></div>'+
		'<div id="ppload" style="height: 100%;">'+
	...
	HIDE_waiting(){ var x =document.getElementById('my_waiting_box'); x.parentNode.removeChild(x); },
	// =========================================================================================================

		 

	// ===================================  show BLACKened background (separate) =============== NOTE: on your over element, use z-index:9600 ) ===============
	SHOW_blackGROUND()  { var AAADIV = document.createElement('div'); AAADIV.id = "my_black_bck_123";  var stl='background-color:black; height:5000px; left:0px; position:fixed; top:0px; width:100%; z-index:9503;';  AAADIV.setAttribute("style", stl ); if (mybodyyy = document.body)  {mybodyyy.insertBefore(AAADIV, mybodyyy.childNodes[0]);}   else {document.write(AAADIV.outerHTML);}                                                            if (typeof SetFadingOKK != "undefined"){ repetrrr44(AAADIV,0);} },   repetrrr44(AAADIV,i){ i =(i || 0)+1; if (i>9) return;  document.getElementById("my_black_bck_123").style.opacity = i/10; window.setTimeout(function(){repetrrr44(AAADIV,i);},30);},
	REMOVE_blackGROUND()  { var AAADIV = document.getElementById('my_black_bck_123'); AAADIV.parentNode.removeChild(AAADIV); },


	PLEASE_WAIT_Box(Show_or_Hide, Loading_Message){
		if(Show_or_Hide){ 
	...
	myyAjaxRequest(parameters, url, method, YOUR_PASSED_CODES, ShowBlackground){ ShowBlackground= ShowBlackground || false;
	...
	*/
	...
	radiobox_onchange_hider(selector,desiredvalue, target_hidding_selector, SHOW_or_hide){
		...
		else 													{if(!SHOW_or_hide) x.show(); else x.hide();}
	},
	//
	...
	loadJquery()
	{
		...
		var cdn = "ajax.googleapis.com";
		script.src = 'https://'+cdn+'/ajax/libs/jquery/1.7.1/jquery.min.js';
		script.type = 'text/javascript';
	...
	MakeFixed(element, ExtraHeightToBody , AddPadding ){
		var ExtraHeightToBody= typeof ExtraHeightToBody !== 'undefined' ?  ExtraHeightToBody : 0;
		var AddPadding 		= typeof AddPadding === 'undefined' ? true : false;
		
		var sticky = $(element);		var StickyHeight=parseInt(sticky.height());
		...
		$(window).scroll(function(){
		  ...
		  var navHeight	= AddPadding ? StickyHeight + ExtraHeightToBody : 0;  
		  this_.Append_To_Head("style", ' body .fixedd {	position:fixed; top:0; left:0;  margin:0 auto; 	width:100%; z-index:99; }');
		  if (scrollTop > navPosition.top)	{if(!sticky.hasClass('fixedd')){sticky.addClass('fixedd');}		b.style.paddingTop= navHeight + "px"; }
		  else 								{if(sticky.hasClass('fixedd')){sticky.removeClass('fixedd');}	b.style.paddingTop=null;  }
		});
	},
	// =============================================================================================================//
	...
	random_number_minmax(min, max) {  return Math.floor(Math.random() * (max - min + 1)) + min;},
	random_text(length)	{ return randomString(length); },
...
// show if loaded
if (isset($_GET['JS'])) { exit(PuvoxSoftware_JS()); }
#endregion
//==========================================================================================================
//==========================================     ### JS codes     =========================================
//==========================================================================================================
...
class standard_wp_library__PuvoxSoftware extends standard_php_library__PuvoxSoftware  
{
	...
	public function init_properties()
	{
		...
		if ($this->property('plugin_disable_update')) 		$this->init__disableupdate();
		...
		if ($this->property("enable_write_logs")) { $this->SaveLogs( dirname(dirname(__DIR__)) .'/___logs_' ); }
		if ($this->property('remove_comments_url')) 		$this->removeUrlsInComments();
	}
	...
	public function removeUrlsInComments()
	{
	...
	public function tinymce_js( $plugin_array )	 {
		return  array_merge($plugin_array,   ["button_handle_" . $this->slug=> $this->homeURL  . '?tinymce_buttons_'.$this->slug] ); 
	}
...
class default_plugin__PuvoxSoftware 
{
	...
	public function my_plugin_vars($step=0)
	{
		...
		[
			...
			'donate_url'		=> 'https://paypal.me/Puvox', // business: http://paypal.me/Puvox   ||  personal : http://paypal.me/ttodua
		...
		if(method_exists($this,'declare_settings') ) $this->declare_settings(); 	//removed "is_admin" because it fails on front-end /wp-json/wp/v2/posts/8?_locale=user
	...
	*/
	...
	public function images($which, $type="png", $url_or_tag=true)
	{
		...
		switch ($which)
		{
			case "overlay-pro"		: $url['png']='https://i.imgur.com/MNxlU7s.png';  $url['svg'] = '<svg height="15pt" preserveAspectRatio="xMidYMid meet" viewBox="0 0 14 15" width="14pt" xmlns="http://www.w3.org/2000/svg"><g transform="matrix(.1 0 0 -.1 0 15)"><path d="m20 125c-13-14-21-27-18-30 2-3 17 9 33 25 16 17 24 30 19 30-6 0-21-11-34-25z"/><path d="m53 91c-73-80-67-94 7-17 33 35 60 66 60 69 0 16-18 2-67-52z"/><path d="m85 50c-27-28-45-50-39-50 13 0 99 88 93 95-3 2-27-18-54-45z"/><path d="m125 10c-3-5-1-10 4-10 6 0 11 5 11 10 0 6-2 10-4 10-3 0-8-4-11-10z"/></g></svg>';  break;
			case "questionMark-1"	: $url['png']='https://i.imgur.com/6oHljXM.png';  $url['svg'] = '<svg height="64pt" preserveAspectRatio="xMidYMid meet" viewBox="0 0 64 64" width="64pt" xmlns="http://www.w3.org/2000/svg"><g transform="matrix(.1 0 0 -.1 0 64)"><path d="m308 602c41-7 42-8 28-28-10-14-31-23-61-27-71-10-85-22-83-65l1-37 4 35c6 42 13 47 81 56 73 9 137-16 159-61 21-43 13-69-42-139-25-32-45-67-45-77 0-15-7-19-35-19-31 0-35 3-35 26 0 30 14 60 46 94 35 39 40 74 13 94-13 9-30 16-39 16-28 0-90-21-90-30 0-5 7-7 16-3 14 5 14 3 2-18-7-12-21-32-30-43-27-30-22-38 12-21 54 28 44 15-17-22-34-21-82-57-108-81l-46-43 31-49c154-240 528-151 557 134 6 57-12 146-29 146-5 0-6 5-3 10 3 6 1 10-6 10s-10 3-6 6c10 11-87 97-132 117-28 12-73 21-116 23-52 2-59 1-27-4zm52-432v-40h-45-45v40 40h45 45z"/><path d="m23 320c0-30 2-43 4-27 2 15 2 39 0 55-2 15-4 2-4-28z"/><path d="m31 244c0-11 3-14 6-6 3 7 2 16-1 19-3 4-6-2-5-13z"/></g></svg>'; break;
			case "questionMark-2"	: $url['png']='https://i.imgur.com/73R7eLv.png';  $url['svg'] = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:se="http://svg-edit.googlecode.com" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape" width="220.36365738125096" height="219.96970986050064" style=""><rect id="backgroundrect" width="100%" height="100%" x="0" y="0" fill="#FFFFFF" stroke="none" class="" style=""/>                                <g class="currentLayer" style=""><title>Layer 1</title><path fill="#4a90d6" fill-opacity="1" stroke="#ebeb1a" stroke-opacity="1" stroke-width="0" stroke-dasharray="none" stroke-linejoin="round" stroke-linecap="butt" stroke-dashoffset="" fill-rule="nonzero" opacity="1" marker-start="" marker-mid="" marker-end="" d="M0,110 C0,49.22651933701658 49.226519337016555,0 110,0 C170.77348066298345,0 220,49.22651933701658 220,110 C220,170.77348066298345 170.77348066298345,220 110,220 C49.226519337016555,220 0,170.77348066298345 0,110 z" id="svg_1" class="" filter=""/><foreignObject fill="#4a90d6" stroke="#222222" stroke-width="2" stroke-linejoin="round" stroke-dashoffset="" fill-rule="nonzero" font-size="156" font-family="Arial, Helvetica, sans-serif" letter-spacing="0" word-spacing="0" marker-start="" marker-mid="" marker-end="" id="svg_9" x="23.115700873212546" y="29.279336103256995" width="224.30167929102208" height="168.0040174784078" style="color: rgb(36, 36, 36);" class="" transform="rotate(0.08801647275686264 832.5809326171628,364.08416748049103) "><p style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;" xmlns="http://www.w3.org/1999/xhtml"><p xmlns="http://www.w3.org/1999/xhtml" style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;"></p><p xmlns="http://www.w3.org/1999/xhtml" style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;"></p><p xmlns="http://www.w3.org/1999/xhtml" style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;"> ?</p><p style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;"></p><p style="border: none;outline: none;font-size: inherit;line-height: 1em;padding:0;margin:0;"></p></p></foreignObject></g><defs><marker id="DotS" refX="0" refY="0" orient="auto" inkscape:stockid="DotS" overflow="visible"> <path transform="scale(.2) translate(7.4 1)" d="M-2.5-1c0 2.76-2.24 5-5 5s-5-2.24-5-5 2.24-5 5-5 5 2.24 5 5z" fill-rule="evenodd" stroke="#000" stroke-width="1pt" style="fill: rgb(235, 235, 26); stroke: rgb(235, 235, 26); stroke-dasharray: none;"/></marker></defs></svg>'; break;
			// https://imgur.com/mx70WNM
			case "rating-transparent" : $url['png']='https://i.imgur.com/73R7eLv.png';  $url['svg'] = '<svg height="128pt" preserveAspectRatio="xMidYMid meet" viewBox="0 0 544 128" width="544pt" xmlns="http://www.w3.org/2000/svg"><g transform="matrix(.1 0 0 -.1 0 128)"><path d="m605 990c-31-60-70-121-86-135l-31-25h29c26 0 33 9 96 135 37 74 63 135 57 135-5 0-34-49-65-110z"/><path d="m1605 990c-31-60-70-121-86-135l-31-25h29c26 0 33 9 96 135 37 74 63 135 57 135-5 0-34-49-65-110z"/><path d="m2605 990c-31-60-70-121-86-135l-31-25h29c26 0 33 9 96 135 37 74 63 135 57 135-5 0-34-49-65-110z"/><path d="m3605 990c-31-60-70-121-86-135l-31-25h29c26 0 33 9 96 135 37 74 63 135 57 135-5 0-34-49-65-110z"/><path d="m4605 990c-31-60-70-121-86-135l-31-25h29c26 0 33 9 96 135 37 74 63 135 57 135-5 0-34-49-65-110z"/><path d="m458 823c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m1458 823c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m2458 823c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m3458 823c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m4458 823c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m378 813c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m1378 813c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m2378 813c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m3378 813c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m4378 813c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m298 803c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m1298 803c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m2298 803c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m3298 803c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m4298 803c7-3 16-2 19 1 4 3-2 6-13 5-11 0-14-3-6-6z"/><path d="m260 763c0-7 30-45 66-85 96-107 101-120 85-226-8-48-15-112-17-142-7-88 11-31 36 118l22 133-59 65c-122 138-132 149-133 137z"/><path d="m1260 763c0-7 30-45 66-85 96-107 101-120 85-226-8-48-15-112-17-142-7-88 11-31 36 118l22 133-59 65c-122 138-132 149-133 137z"/><path d="m2260 763c0-7 30-45 66-85 96-107 101-120 85-226-8-48-15-112-17-142-7-88 11-31 36 118l22 133-59 65c-122 138-132 149-133 137z"/><path d="m3260 763c0-7 30-45 66-85 96-107 101-120 85-226-8-48-15-112-17-142-7-88 11-31 36 118l22 133-59 65c-122 138-132 149-133 137z"/><path d="m4260 763c0-7 30-45 66-85 96-107 101-120 85-226-8-48-15-112-17-142-7-88 11-31 36 118l22 133-59 65c-122 138-132 149-133 137z"/><path d="m710 356c19-7 77-34 127-60 51-25 96-46 100-45 17 1-223 118-242 118-13 0-7-5 15-13z"/><path d="m1710 356c19-7 77-34 127-60 51-25 96-46 100-45 17 1-223 118-242 118-13 0-7-5 15-13z"/><path d="m2710 356c19-7 77-34 127-60 51-25 96-46 100-45 17 1-223 118-242 118-13 0-7-5 15-13z"/><path d="m3710 356c19-7 77-34 127-60 51-25 96-46 100-45 17 1-223 118-242 118-13 0-7-5 15-13z"/><path d="m4710 356c19-7 77-34 127-60 51-25 96-46 100-45 17 1-223 118-242 118-13 0-7-5 15-13z"/></g></svg>'; break;
		}
	}
