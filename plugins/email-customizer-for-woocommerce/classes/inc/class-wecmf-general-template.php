...
class WECMF_General_Template{
	...
	public function check_nonce(){
		...
		if ( isset( $_POST['i_edit_template'] ) && $template ){
	...
	public function save_template_content(){
		$response = '';
		if( WECMF_Email_Customizer_Utils::is_valid_action() ){
			...
			if( $action_index && $action_index == 'settings' && WECMF_Email_Customizer_Utils::wecm_valid() ){
	...
	public function insert_dynamic_data($template_html_final){
		...
		$modified_data = str_replace('<span class="thwecmf_before_shipping_table"></span>', $this->add_order_head( true ), $modified_data);
		$modified_data = str_replace('<span class="thwecmf_after_shipping_table"></span>', $this->add_order_foot( true ), $modified_data);
	...
	public function replace_thwecmf_placeholder_data($modified_data){
		...
		* Placeholders made compatible with the premuim version
		...
		$modified_data = str_replace('{reset_password_url}', $this->wecmf_get_reset_password_url(), $modified_data);
		$modified_data = str_replace('{customer_note}', $this->wecmf_get_customer_note(), $modified_data);
	...
	public function wecmf_get_reset_password_url(){
	...
	public function wecmf_get_customer_note(){
