...
class WECMF_Builder_Settings extends WECMF_Settings_Page {
	...
	private function get_template_data($t_name){
		...
		if($t_name){
			...
			if( WECMF_Email_Customizer_Utils::wecm_valid( $t_name, true ) ){
				$template_data = isset( $t_list['templates'][$t_name] ) ? $t_list['templates'][$t_name] : WECMF_Email_Customizer_Utils::thwecmf_get_templates($t_name);
			}else{
				$this->thwecmf_invalid_template();
			}
			$this->template_display_name = isset ( $template_data['display_name'] ) ? $template_data['display_name'] : '';
			$template_data = isset( $template_data['template_data'] ) ? $template_data['template_data'] : '';
		...
		<div class="thwecmf-tbuilder-header-action-left">
			<div class="thwec-new-template-icon thwecmf-header-icons">
				...
				<div class="thwecmf-header-icons-holder thwecmf-icon-box thwecmf-header-tab" onclick="thwecBuilderPreviewTab(this)" data-tab="preview">
					...
					<div class="thwecmf-header-icons-holder thwecmf-icon-box thwecmf-template-action-button" id="thwec_template_save_button">
						<span class="thwecmf-header-icon-inner thwecmf-header-button" onclick="thwecmfSaveTemplate(this)">
							...
							<input type="hidden" id="thwecmf_template_save_name" name="thwecmf_template_name" value="<?php echo $this->template_display_name; ?>">
						</span>
					</div>
				...
				<div class="thwecmf-header-icons-holder thwecmf-icon-box thwecmf-test-mail-box">
					<span class="thwecmf-header-icon-inner thwecmf-header-button" onclick="thwecmfClickTestMail(this, 'test_mail')">
	...
	private function output_builder_editor_panel($template_json) {
		...
		<div class="template-wrapper-settings thwecmf-icon-wrapper">
			<img src="<?php echo TH_WECMF_ASSETS_URL.'images/settings-cogwheel.svg';?>" onclick="thwecmfEditBuilderBlocks(this, 't_builder', 't_builder')" alt="Template Builder Settings" title="Template Builder Settings">
		</div>
	...
	private function render_builder_elm_pp_fragment_border($content=false,$prefix='', $premium_disable=false){
    ...
    public function render_form_fields($field, $atts = array(), $render_cell = true, $bypass_premium = false){
		...
		if($field && is_array($field)){
			...
			if($render_cell && $render_cell !== 'template-map'){
				...
				<?php } ?>
				<tr>
					<td class="<?php echo esc_html( $additional_data ); ?>" >
						<?php echo $field_html; ?>
					</td>
				</tr>
				<?php
		...
		<div id="thwecmf_modal_test_mail" style="display: none;">
			...
			<div class="thwecmf-modal-body">
				...
				<div class="thwecmf-test-mail-modal">
					<input type="text" class="thwecmf-test-mail-input" name="test_mail_input" placeholder="Enter an email id" value="<?php echo $user_email; ?>">
