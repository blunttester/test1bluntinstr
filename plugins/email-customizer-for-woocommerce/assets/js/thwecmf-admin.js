var thwecmf_settings = (function($, window, document) {
    ...
    function tbuilder_initialize(){ 
        ...
        $('#wecmf_email_template_manager_table .thwecmf-template-manage-tabs').click(function(){
            if( !$(this).hasClass('thwecmf-template-manage-active') ){
                var tab = $(this).attr('data-name');

                var content = tab == 'manage' ? $('#thwecmf_manage_template_table tbody').html() : $('#thwecmf_map_template_table tbody').html();
                $('#wecmf_email_template_manager_table > tbody').html( content );
                if( tab == 'mapping' ){
                    setup_select2_template_map();
                }
                $('#thwecmf_template_manager').find('input[name="thwec_notification_tab"]').val(tab);
                $(this).siblings('.thwecmf-template-manage-active').removeClass('thwecmf-template-manage-active');
                $(this).addClass('thwecmf-template-manage-active');
            }
        });

        $('#tbf_t_builder').on('click', '.thwecmf-block', function(event) {
            ...
            render_edit_block_sidebar_form(builder_elm, panel_elm_id, builder_elm.attr('data-block-name') );
        });

        $('#wecmf_email_template_manager_table').on('click', '.thwecmf-reset-link', function(event) {
            if( !confirm( 'Template will be reset to initial condition. Do you want to proceed ?' ) ){
                event.preventDefault();
            }
        });
    }

    function setup_select2_template_map(){
        var select2 = $('#wecmf_email_template_manager_table .thwecmf-mapping-table');
        select2.find('.thwecmf-template-map-select2.select2-hidden-accessible').select2('destroy');
        select2.find('.thwecmf-template-map-select2').select2();
    }
    ...
    function click_test_mail_button(elm){
        var content = $('#thwecmf_modal_test_mail').html();
        var modal_box = $('#thwecmf_builder_header_action_box');
        remove_builder_panel_highlights();
        modal_box.find('.content-box').html(content);
        modal_box.addClass('thwecmf-modal-active');
    }

    function remove_builder_panel_highlights(){
        $('#thwecmf-sidebar-configure .thwecmf-builder-elm-layers').find('.thwecmf-panel-highlight').removeClass('thwecmf-panel-highlight');
        $('#tbf_t_builder').find('.thwecmf-builder-highlight').removeClass('thwecmf-builder-highlight');
    }

    function click_test_mail_action(elm){
        var button = $(elm);
        var mail_id = button.siblings('input[name="test_mail_input"]').val();
        var validation_box = button.closest('.thwecmf-test-mail-modal').siblings('.thwecmf-action-validation-box');
        ...
        if(valid_result['valid']){
            ...
            prepare_ajax_test_email(test_mail_data, css, mail_id,validation_box);
        }else{
            var error_msgs = '';
            $.each(valid_result['errors'], function(key, value) {
                 error_msgs += prepare_validation_html( value, valid_result['valid'] );
            });
           validation_box.html( error_msgs );
        }
    }

    function prepare_validation_html( message, valid ){
        var html = '';
        var valid_class = valid ? 'thwecmf-validation-success' : 'thwecmf-validation-failed';
        html+= '<p class="'+valid_class+'">'+message+'</p>';
        return html;
    }

    function prepare_ajax_test_email(mail_data, mail_css, mail_id, msg_box ){
    ...
    function setup_sidebar_scrolls(active_tab1){
        ...
        var height1 = sidebar_wrapper.height() - 55;
    ...
    *---------------------------------*/
    ...
    function render_edit_block_sidebar_form(elm, blockId, blockName){
        ...
        reset_sidebar_status(false);
        var form = $('#thwecmf-sidebar-settings').find('form');
    ...
    function save_sidebar_edit_form(field, event){
        var form = $('#thwecmf-sidebar-settings').find('form');
        ...
        data_json_text = data_json_text ? data_json_text : '{}';
        ...
        var value = get_form_field_value(field, event.target.type);
        if( key in ELM_FORM_PROPS ){
            key = ELM_FORM_PROPS[key];
        }
        var style_props = key in EDIT_FORM_PROPS ? EDIT_FORM_PROPS[key] : false;
        if( style_props ){
            if(key in data_css){
                if(typeof style_props['attribute'] != 'undefined'){
                    if(key == 'textarea_content'){ 
                        var cleaned_textarea = thwecmfEscapeHTML( escapeScripts( value ) );
                        ...
                        if( key == 'content' ){
                            ...
                            css_props[key] = '';
                            data_json_text[key] = thwecmfEscapeHTML(thwecmf_sanitize_field( value ));
                            data_json_css[key] = '';
                        }else if(key === 'upload_img_url'){
                            var display_css = (value !== '') ? 'inline-block' : 'none';
                            js_props[key] = value;
                            css_props[key] = display_css;
                            data_json_text[key] = value;
                            data_json_css[key] = display_css;
                        }
                        js_props[key] = value;
                    }
                }else{
            ...
            if( !$.isEmptyObject( css_props ) ){
                if( key in BUILDER_CSS_PROPS ){
                    var css_prop = BUILDER_CSS_PROPS[key]['name'];
                    var css_value =  css_props[key] != '' ? css_props[key] : DEFAULT_CSS[css_prop];
                    if( !isCssValid( css_prop, css_value ) ){
                        return;
                    }
                }
            }
            if(sidebar_form_valid.length === 0){    // Validation only returns on status. If success then no returns. If changing the code, keep an eye here
                ...
                if( !$.isEmptyObject( data_json_text ) && js_props ){
                ...
                if(block_name == 'text' && key == 'textarea_content'){
                    js_props[key] = form_textarea_manipulate(js_props[key]);
                }
                prepare_builder_block_css(key, css_props, block_id, block_name);
    ...
    function prepare_builder_block_css(key, props, block_id, block_name){
        var tb_css_override_elm = $('#thwecmf_template_css_override');
        var tb_css_override = tb_css_override_elm.html();
        tb_css_override += prepare_builder_css_override(key, props, 'tbf_'+block_id, block_name, true);
        ...
        prev_css_override += prepare_builder_css_override(key, props, 'tpf_'+block_id, block_name, true);
        prev_css_override_elm.html(prev_css_override);
    }

    function prepare_builder_css_override(key, props, blockId, blockName, isPrev){
        var css = '';
        switch(blockName) {
            case 'one_column':
            case 'two_column':
                css = prepare_layout_row_css_override(key, props, blockId, isPrev);
                break;
            case 'one_column_one':
            case 'two_column_one':
            case 'two_column_two':
                css = prepare_layout_col_css_override(key, props, blockId, isPrev);
                break;
            case 'text':
                css = prepare_block_css_override_text(key, props, blockId, blockName, isPrev);
                break;
            case 'billing_address':
                css = prepare_block_css_override_billing_address(key, props, blockId, blockName, isPrev);
                break;
            case 'shipping_address':
                css = prepare_block_css_override_shipping_address(key, props, blockId, blockName, isPrev);
                break;
            case 'image':
                css = prepare_block_css_override_image(key, props, blockId, blockName, isPrev);
                break;
            case 'gap':
                css = prepare_block_css_override_gap(key, props, blockId, blockName, isPrev);
                break;
            case 'divider':
                css = prepare_block_css_override_divider(key, props, blockId, blockName, isPrev);
                break;
             case 't_builder':
                css = prepare_block_css_override_t_builder(key, props, blockId, blockName, isPrev);
                break;
            default:
                css = '';
        }
    ...
    function prepare_layout_row_css_override(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-row"; 
        var row_css = '';

        var row_props_arr = ['height', 'p_t', 'p_r', 'p_b', 'p_l', 'm_t', 'm_r', 'm_b', 'm_l', 'b_t', 'b_r', 'b_b', 'b_l', 'border_style', 'border_color', 'bg_color'];
        if( $.inArray( key, row_props_arr ) !== -1 ){
            row_css = w_selector+' {';
            row_css += builder_css_generator( props );
            row_css += '}';
        }
        return row_css;
    }

    function prepare_layout_col_css_override(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-column-padding";
        var col_css = '';

        var col_props = ['width', 'text_align', 'p_t', 'p_r', 'p_b', 'p_l', 'b_t', 'b_r', 'b_b', 'b_l', 'border_style', 'border_color', 'bg_color'];
        if( $.inArray( key, col_props ) !== -1 ){
            col_css = w_selector+'{';
            col_css += builder_css_generator(props);
            col_css += '}';
        }
        return col_css;
    }

    function prepare_block_css_override_text(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-text";
        var tb_css = '';
        var td_css = '';
        var content_css = '';

        var tb_props = ['color', 'align', 'font_size', 'size_width', 'size_height', 'm_t', 'm_r', 'm_b', 'm_l', 'text_align'];
        if( $.inArray( key, tb_props ) !== -1 ){
            tb_css = w_selector+'{';
            tb_css += builder_css_generator(props);
            tb_css += '}';
        }

        var td_props = ['color', 'font_size', 'text_align', 'bg_color', 'b_t', 'b_r', 'b_b', 'b_l', 'border_color', 'border_style', 'p_t', 'p_r', 'p_b', 'p_l'];
        if( $.inArray( key, td_props ) !== -1 ){
            td_css = w_selector+' .thwecmf-block-text-holder{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }

        var content_props = ['color', 'font_size'];
        if( $.inArray( key, content_props ) !== -1 ){
            content_css = w_selector+' *{';
            content_css += builder_css_generator(props);
            content_css += '}';
        }
        return tb_css+td_css+content_css;
    }

    function prepare_block_css_override_billing_address(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-billing";
        var align_css = '';
        var tb_css = '';
        var td_css = '';
        var header_css = '';
        ...
        var align_props = ['align'];
        if( $.inArray( key, align_props ) !== -1 ){
            align_css = w_selector+' .thwecmf-address-alignment{';
            align_css += builder_css_generator(props);
            align_css += '}';
        }

        var tb_props = ['size_width', 'size_height', 'bg_color', 'b_t', 'b_r', 'b_b', 'b_l', 'border_style', 'border_color', 'm_t', 'm_r', 'm_b', 'm_l'];
        if( $.inArray( key, tb_props ) !== -1 ){
            td_css = w_selector+' .thwecmf-address-wrapper-table{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }

        var td_props = ['p_t', 'p_r', 'p_b', 'p_l'];
        if( $.inArray( key, td_props ) !== -1 ){
            td_css = w_selector+' .thwecmf-billing-padding{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }

        var header_props = ['font_size', 'color','text_align'];
        if( $.inArray( key, header_props ) !== -1 ){
            header_css = w_selector+' .thwecmf-billing-header{';
            header_css += builder_css_generator(props);
            header_css += '}';
        }

        var body_props = ['details_font_size', 'details_color','details_text_align'];
        if( $.inArray( key, body_props ) !== -1 ){
            body_css = w_selector+' .thwecmf-billing-body{';
            body_css += builder_css_generator(props);
            body_css += '}';
        }
        return align_css+tb_css+td_css+header_css+body_css;
    }

    function prepare_block_css_override_image(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-image";
        var td_css = '';
        var content_css = '';

        var td_props = ['content_align'];
        if( $.inArray( key, td_props ) !== -1 ){
            td_css = w_selector+' td.thwecmf-image-column{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }

        var content_props = ['img_size_width', 'img_size_height'];
        if( $.inArray( key, content_props ) !== -1 ){
            content_css = w_selector+' td.thwecmf-image-column p{';
            content_css += builder_css_generator(props);
            content_css += '}';
        }
        return td_css+content_css;
    }

     function prepare_block_css_override_shipping_address(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-shipping";
        var align_css = '';
        var tb_css = '';
        var td_css = '';
        var header_css = '';
        ...
        var td_props = ['p_t', 'p_r', 'p_b', 'p_l'];
        if( $.inArray( key, td_props ) !== -1 ){
            td_css = w_selector+' .thwecmf-shipping-padding{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }
        ...
        var header_props = ['font_size', 'color','text_align'];
        if( $.inArray( key, header_props ) !== -1 ){
            header_css = w_selector+' .thwecmf-shipping-header{';
            header_css += builder_css_generator(props);
            header_css += '}';
        }
        ...
        var body_props = ['details_font_size', 'details_color','details_text_align'];
        if( $.inArray( key, body_props ) !== -1 ){
            body_css = w_selector+' .thwecmf-shipping-body{';
            body_css += builder_css_generator(props);
            body_css += '}';
        }
        return align_css+tb_css+td_css+header_css+body_css;
    ...
    function prepare_block_css_override_gap(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-gap";
        var content_css = '';

        var content_props = ['height', 'bg_color', 'b_t', 'b_b', 'b_l', 'b_r',  'border_style', 'border_color'];
        if( $.inArray( key, content_props ) !== -1 ){
            content_css = w_selector+'{';
            content_css += builder_css_generator(props);
            content_css += '}';
    ...
    function prepare_block_css_override_divider(key, props, blockId, isPrev){
         var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-block-divider";
        var tb_css = '';
        var td_css = '';
        var content_css = '';
        ...
        var content_props = ['width', 'divider_height', 'divider_color', 'divider_style'];
        if( $.inArray( key, content_props ) !== -1 ){
            content_css = w_selector+' td hr{';
            content_css += builder_css_generator(props);
            content_css += '}';
        }
        return tb_css+td_css+content_css;
    ...
    function prepare_block_css_override_t_builder(key, props, blockId, isPrev){
        var p_selector = css_parent_selector(blockId, isPrev);
        var w_selector = p_selector+".thwecmf-main-builder";
        var td_css = '';

        var td_props = ['b_t', 'b_r', 'b_b', 'b_l', 'border_style', 'border_color', 'bg_color'];
        if( $.inArray( key, td_props ) !== -1 ){
            td_css = w_selector+' .thwecmf-builder-column{';
            td_css += builder_css_generator(props);
            td_css += '}';
        }
    ...
    function reset_sidebar_status($configure){
        ...
        var sidebar = $('#thwecmf-sidebar-settings');
        if( sidebar.find('.thwecmf-block-settings-form').length == 1 && $configure ){
            var block_id = $('#thwecmf-sidebar-settings').find('input[name="i_thwecmf_block_id"]').val();
            var builder_elm = $('#tbf_'+block_id);
            var panel_elm = $('#'+block_id);
            if( builder_elm.hasClass('thwecmf-block') || builder_elm.hasClass('hook-code') ){
    ...
    function initialize_sidebar_clicks(){
        ...
        $('.thwecmf-tbuilder-wrapper').on('click', '.thwecmf-aligment-icon-wrapper .img-wrapper', function(event) {
            ...
            icon_wrapper.find('.thwecmf-text-align-input').trigger('keyup');
            icon_wrapper.find('.thwecmf-active-icon').removeClass('thwecmf-active-icon');
            icon_wrapper.find('.thwecmf-active-icon').removeClass('thwecmf-active-icon');
            icon_elm.addClass('thwecmf-active-icon');
        });

        $('#thwecmf-sidebar-settings').on('keyup', 'form :input', function( event ){
            save_sidebar_edit_form( $(this), event );
        });

        $('#thwecmf-sidebar-settings').on('change', 'select', function( event ){
            save_sidebar_edit_form( $(this), event );
        });
    }
...
------------------------------------------------------------------------------------------------------------------*/ 
    ...
    function remove_image_uploaded(elm){
        ...
        upload_preview.closest('.thwecmf-upload-action-settings').find('.thwecmf-upload-url').val(default_url).trigger('keyup');
    ...
    function setup_wp_image_upload(elm,prop){ 
        ...
        frame.on( 'select', function() { 
            ...
            var thwecmf_attachment = frame.state().get('selection').first().toJSON();
            if(prop == 'gif'){
                file_valid = thwecmf_attachment['mime'] == 'image/gif' ? true : false;
                validation_msg = '<p>Invalid file type. Choose a Gif file</p>';
            }else{
                file_valid = thwecmf_attachment['type'] == 'image' &&  $.inArray(thwecmf_attachment['mime'], UPLOAD_FILE_TYPES) !== -1 ? true : false;
                validation_msg = '<p>Invalid file type. Choose an image file <br>[ jpg, jpeg, png ]</p>';
            }
            var thwecmf_attachment_url = thwecmf_attachment['url'];
            var form_table = $('#thwecmf-sidebar-settings').find('form');
            var upload_block = form_table.find('.thwecmf-upload-action-settings.thwecmf-img-preview-'+prop);
            if(file_valid){
                if(prop == 'image' || prop == 'gif'){
                    set_field_value(form_table, 'text', 'upload_img_url', thwecmf_attachment_url, 0);
                    form_table.find('input[name="i_upload_img_url"]').trigger('keyup');
                }
                upload_block.find('.thwecmf-upload-preview img').attr('src',thwecmf_attachment_url);
    ...
    function prepare_ajax_save(name, template_data, css_cleaned, json_tree){
        ...
        $.ajax({
            ...
            beforeSend: function(){
                template_ajax_load.addClass("thwecmf-ajax-loading");
                $('#thwecmf_builder_save_messages').html('').removeClass("thwecmf-show-save thwecmf-save-success");
            },
            success:function(data){
                var msg = data ? "Template Saved" : "Template was not Saved. Try again."
                ...
                display_save_messages( msg, msg_class );
            },
            complete: function() {
                template_ajax_load.removeClass("thwecmf-ajax-loading");
            ...
            error: function(){
                display_save_messages('Something went wrong', 'error');
            }
        });
    }


    function display_save_messages( msg, msg_class ){
        var message_box = $('#thwecmf_builder_save_messages');
        message_box.html( msg ).addClass('thwecmf-show-save thwecmf-save-'+msg_class);
        setTimeout(function(){
            message_box.removeClass("thwecmf-show-save thwecmf-save-"+msg_class);
        },3000);
    }
    ...
    function manage_dynamic_address_support(block_elm, block_name){
        if(block_name == "billing_address"){
            hook_position_manager(block_elm,'{billing_address}','thwecmf-billing-body',false,false);
        }
        if(block_name == "shipping_address"){
            hook_position_manager(block_elm,'{shipping_address}','thwecmf-shipping-body',false,false);
    ...
    function close_modal( elm ){
        $('#thwecmf_builder_header_action_box').removeClass('thwecmf-modal-active');
    }

    function map_form_toggle( elm ){
        var button = $(elm);
        var button_class = button.attr('class');
        var map_table = $('#wecmf_email_template_manager_table .thpladmin-form-email-notification-table');
        if( button_class == 'thwecmf-close-all' ){
            map_table.find('.thwecmf-map-toggle-active').removeClass('thwecmf-map-toggle-active');
            button.text('Expand all');
            button.removeClass('thwecmf-close-all').addClass('thwecmf-open-all');
        }else if( button_class == 'thwecmf-open-all' ){
            map_table.find('.template-map-single-row ').addClass('thwecmf-map-toggle-active');
            button.text('Collpase all');
            button.removeClass('thwecmf-open-all').addClass('thwecmf-close-all');
        }
    }
    ...
    function toggle_builder_preview_tab( elm ){
        ...
        var active_tab_elm = tab_elm.closest('.thwecmf-header-icons').find('.active-tab');
    ...
    function switch_to_preview(){
        ...
        template_preview_content(show_hooks); // Check order of functions
    }

    function switch_to_builder(){
        $('#thwecmf-template-builder-wrapper').removeClass('thwecmf-preview-active');
        $('#wpfooter').show();
        $('#thwecmf_tbuilder_editor_preview').hide();
    }

    function hide_builder_sidebar(){
        $('#thwecmf-template-builder-wrapper').addClass('thwecmf-preview-active');
        $('#wpfooter').hide();
        $('#thwecmf_tbuilder_editor_preview').show();
    }
...
function thwecmfClickTestMailAction(elm){
...
function thwecmfMapFormToggle( elm ){
    thwecmf_settings.map_form_toggle( elm );
}

function thwecmfCloseModal( elm ){
    thwecmf_settings.close_modal( elm );
}

function thwecBuilderPreviewTab( elm ){
    thwecmf_settings.toggle_builder_preview_tab( elm );
}